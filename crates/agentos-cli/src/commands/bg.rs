...
pub async fn handle(client: &mut BusClient, command: BgCommands) -> anyhow::Result<()> {
    match command {
        BgCommands::Run { name, agent, task, detach } => {
            ...
            if let KernelResponse::Success { data } = response {
                let task_id = data.unwrap().get("task_id").unwrap().as_str().unwrap().to_string();
        ...
        BgCommands::List => {
            ...
            if let KernelResponse::BackgroundPoolList(tasks) = response {
                ...
                println!("{:<20} {:<15} {:<15} {:<20} {:<20}", "NAME", "AGENT", "STATE", "STARTED", "COMPLETED");
                for task in tasks {
                    let started = task.started_at.format("%Y-%m-%d %H:%M:%S").to_string();
                    let completed = task.completed_at.map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string()).unwrap_or_else(|| "N/A".to_string());
                    let state = format!("{:?}", task.state).to_lowercase();
                    println!("{:<20} {:<15} {:<15} {:<20} {:<20}",
        ...
        BgCommands::Logs { name, follow } => {
            let response = client.send_command(KernelCommand::GetBackgroundLogs { name: name.clone(), follow }).await?;
        ...
        BgCommands::Kill { name } => {
            let response = client.send_command(KernelCommand::KillBackground { name: name.clone() }).await?;
