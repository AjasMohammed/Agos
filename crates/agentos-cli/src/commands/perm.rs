...
pub async fn handle(client: &mut BusClient, command: PermCommands) -> anyhow::Result<()> {
    match command {
        PermCommands::Grant { agent, permission, expires } => {
            let response = if let Some(secs) = expires {
                client.send_command(KernelCommand::GrantPermissionTimed {
                    agent_name: agent.clone(),
                    permission: permission.clone(),
                    expires_secs: secs,
                }).await?
            } else {
                client.send_command(KernelCommand::GrantPermission {
                    agent_name: agent.clone(),
                    permission: permission.clone(),
                }).await?
            };
            match response {
                KernelResponse::Success { .. } => println!("✅ Granted permission '{}' to agent '{}'", permission, agent),
                KernelResponse::Error { message } => eprintln!("❌ Error: {}", message),
                _ => eprintln!("❌ Unexpected response"),
            }
        }
        PermCommands::Revoke { agent, permission } => {
            let response = client.send_command(KernelCommand::RevokePermission {
                agent_name: agent.clone(),
                permission: permission.clone(),
            }).await?;
            match response {
                KernelResponse::Success { .. } => println!("✅ Revoked permission '{}' from agent '{}'", permission, agent),
                KernelResponse::Error { message } => eprintln!("❌ Error: {}", message),
                _ => eprintln!("❌ Unexpected response"),
            }
        }
        PermCommands::Show { agent } => {
            let response = client.send_command(KernelCommand::ShowPermissions {
                agent_name: agent.clone(),
            }).await?;
            match response {
        ...
        PermCommands::Profile { command } => {
            match command {
                PermProfileCommands::Create { name, description, permissions } => {
                    let response = client.send_command(KernelCommand::CreatePermProfile {
                        name: name.clone(),
                        description,
                        permissions,
                    }).await?;
                ...
                PermProfileCommands::Delete { name } => {
                    let response = client.send_command(KernelCommand::DeletePermProfile { name: name.clone() }).await?;
                    match response {
                ...
                PermProfileCommands::Assign { agent_name, profile_name } => {
                    let response = client.send_command(KernelCommand::AssignPermProfile {
                        agent_name: agent_name.clone(),
                        profile_name: profile_name.clone(),
                    }).await?;
                    match response {
                        KernelResponse::Success { .. } => println!("✅ Assigned profile '{}' to agent '{}'", profile_name, agent_name),
                        KernelResponse::Error { message } => eprintln!("❌ Error: {}", message),
                        _ => eprintln!("❌ Unexpected response"),
                    }
                }
