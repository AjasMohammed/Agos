pub mod adapters;
pub mod agent_inbox;
pub mod agent_inbox_prompt;
pub mod agent_inbox_writer;
pub mod agent_message_bus;
pub mod agent_message_inbox;
pub mod agent_registry;
pub mod approval_policy_store;
pub mod attachment_sink;
pub mod background_pool;
pub mod capability_broker;
pub mod capability_dispatch;
pub mod capability_provider;
pub mod capability_registry;
pub mod channel_chat_bridge;
pub mod chat_store;
pub mod checkpoint_store;
pub mod claude_mcp_gateway;
pub mod commands;
pub mod config;
pub mod config_watcher;
pub mod consolidation;
pub mod context;
pub mod context_compactor;
pub mod context_compiler;
pub mod context_injector;
pub mod context_memory_store;
pub mod convo_store;
pub mod core_manifests;
pub mod cost_tracker;
pub mod delivery_router;
pub mod escalation;
pub mod escalation_channel_sink;
pub mod event_bus;
pub mod event_dispatch;
pub mod event_permissions;
pub mod fallback_resolver;
pub mod file_store;
pub mod health;
pub mod health_monitor;
pub mod hooks;
pub mod identity;
pub mod inbound_router;
pub mod injection_scanner;
pub mod intent_validator;
pub mod interest_model;
pub mod kernel;
pub mod kernel_action;
pub mod logging;
pub mod managed_build;
pub mod managed_env;
pub mod managed_network;
pub mod managed_process;
pub mod managed_storage;
pub mod mcp_attachment_store;
pub mod mcp_catalog;
pub mod mcp_oauth_provider;
pub mod media_download;
pub mod memory_blocks;
pub mod memory_extraction;
pub mod metrics;
pub mod network_safety;
pub mod notification_router;
pub mod otel_exporter;
pub mod output_sanitizer;
pub mod personalization_feedback;
pub mod plugin_registry;
pub mod policy_engine;
pub mod rate_limit;
pub mod recommendation_engine;
pub mod recommendations_store;
pub mod resource_arbiter;
pub mod retrieval_gate;
pub mod risk_classifier;
pub mod router;
pub mod rpc_manager;
pub mod run_loop;
pub mod runtime_resolver;
pub mod schedule_action_policy;
pub mod schedule_manager;
pub mod schedule_persistence;
pub mod schedule_store;
pub mod scheduler;
pub mod schema_registry;
pub(crate) mod sd_notify;
pub mod skill_installer;
pub mod snapshot;
pub mod state_store;
pub mod system_prompt;
pub mod task_completion;
pub mod task_executor;
pub(crate) mod task_summary;
pub mod task_trace;
pub mod tool_call;
pub mod tool_registry;
pub mod tool_scoping;
pub mod tool_usage_store;
pub mod trace_collector;
pub mod transcription;
pub mod trigger_prompt;
pub mod user_channel_registry;
pub mod user_inbox;
pub mod user_interests_store;
pub mod user_pref_proposals;
pub mod user_profile_store;
pub mod webhook_batcher;
pub mod webhook_registry;
pub mod webhook_throttle;
pub mod webhook_verify;
pub mod webhook_wakeup;
pub mod workspace_grant_store;
pub mod workspace_store;

pub use agent_message_bus::AgentMessageBus;
pub use agent_registry::AgentRegistry;
pub use capability_provider::{CapabilityContext, CapabilityProvider, CapabilityResult};
pub use capability_registry::CapabilityRegistry;
pub use config::{load_config, KernelConfig};
pub use context::ContextManager;
pub use context_compiler::ContextCompiler;
pub use kernel::{resolve_boot_vault_passphrase, ChatStreamEvent, Kernel};
pub use otel_exporter::OtelExporter;
pub use retrieval_gate::{
    IndexType, RetrievalExecutor, RetrievalGate, RetrievalOutcome, RetrievalPlan, RetrievalResult,
};
pub use scheduler::TaskScheduler;
pub use tool_call::{parse_intent_type, ParsedToolCall, ToolCallRequest};
pub use tool_registry::ToolRegistry;
