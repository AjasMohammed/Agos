...
| 02 | [[02-Installation and First Run]] | Signed release installer (`install.sh`, minisign), building from source, `dist` and lite builds, configuration, first kernel boot |
| 03 | [[03-Architecture Overview]] | System architecture, crate dependency graph, the intent flow from CLI to tool execution |
| 04 | [[04-CLI Reference Complete]] | All 44 `agentos` command groups with flags, arguments, and examples — includes `team`, `skill`, `mcp` (attach/detach/oauth-store/a2a-*), `a2a` (card/discover/delegate/tasks), `workspace`, `provider`, `plugin`, `onboard`, `doctor`, `config`, `init`, plus `start`, `stop`, `scratchpad`, `healthz`, `log`, `notifications`, `channel`, and `web` |
...
| 07 | [[07-Tool System]] | All 136 built-in tools — file I/O, memory tiers, scratchpad, multi-agent coordination, task management, HAL device tools, agent inbox (6 tools), scheduling (11), event self-subscription (4), channels & artifacts, user files, containers, IoT, tool discovery/pagination (4), host introspection (4), manifests, trust tiers, signing |
| 08 | [[08-Security Model]] | 8 core defense layers (+ KMC), capability tokens, permission enforcement, injection scanner, risk levels, manifest `risk_class` × approval-mode matrix, per-server MCP grants (`mcp:<server>/`), host-folder grants (`fs.workspace`), remembered approvals, hooks (`AuditHook`, `ApprovalHook`) |
...
| 12 | [[12-Event System]] | 95 event types in 11 categories (incl. `ChatEvents`), subscriptions, filter predicates, event-triggered tasks, throttle policy, reaction batching, agent self-subscription |
| 13 | [[13-Cost Tracking]] | Per-agent token costs, budget enforcement, model pricing table, cost CLI |
| 14 | [[14-Audit Log]] | 173 event types — task lifecycle, MCP, OAuth, containers, IoT device twins, checkpoint recovery, append-only SQLite chain, Merkle verification, export, snapshots |
...
| 16 | [[16-Configuration Reference]] | Every config key in `config/default.toml` with type, default value, and description — incl. `[tools.discovery]`, `[chat]`, `[gateway]`, `[transcription]`, `[personalization]`, `[agent_heartbeat]` |
...
| 22 | [[22-MCP Integration]] | Bidirectional MCP bridge — boot-time servers, runtime `mcp attach`/`detach`, persisted across restarts, OAuth credential lifecycle, A2A (Agent-to-Agent) protocol, per-server permission grants, security gate (injection scanning + rate limit) |
| 23 | [[23-REST API Reference]] | 135 paths / 177 operations under `/api/v1/*` (full route index generated from `openapi.json`) plus the OpenAI-compatible `/v1/chat/completions` SSE endpoint — auth, permissions, request/response shapes, rate limiting, error codes |
...
| 26 | [[26-Channel Adapters]] | All 10 bidirectional messaging adapters — Discord, Telegram, Slack, Matrix, Mattermost, Teams, LINE, WhatsApp, Email, Webhook — inbound routing, slash commands (`/approve`, `/pair`, `/agent`, …), media, pairing manager, health monitor, retry with backoff |
